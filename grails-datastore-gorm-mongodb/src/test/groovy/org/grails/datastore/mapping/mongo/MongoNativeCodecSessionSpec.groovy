...
class MongoNativeCodecSessionSpec extends GormDatastoreSpec {
    ...
    void "test native session is created for native transactions"() {
        ...
        Person.withTransaction { status ->
            ...
            tx = session.getCurrentTransaction()
        }
    ...
    void "test bulk deleteAll with native session"() {
        ...
        Person.withTransaction { status ->
            ...
            countDuringTransaction = Person.countByLastName("Test")
        }

        then: "delete should be immediate"
        deletedCount == 5
        countDuringTransaction == 0
    ...
    void "test bulk updateAll with native session"() {
        ...
        long updatedCount = 0
        boolean allUpdatedDuringTransaction = false
        Person.withTransaction { status ->
            updatedCount = Person.where { lastName == "Test" }.updateAll(age: 35)
            allUpdatedDuringTransaction = Person.findAllByLastName("Test").every { it.age == 35 }
        }

        then: "update should be immediate"
        updatedCount >= 0 // May return -1 on older MongoDB versions
        allUpdatedDuringTransaction
    ...
    void "test association handling in bulk operations"() {
        ...
        def ownerIdDuringTransaction = null
        Person.withTransaction { status ->
            Pet.where { name == "Fluffy" }.updateAll(owner: person)
            ownerIdDuringTransaction = Pet.findByName("Fluffy").owner.id
        }

        then: "association should be handled correctly"
        ownerIdDuringTransaction == person.id
    }
    ...
    void "test concurrent native transactions"() {
        ...
        def results = []
        def countDuringNestedTransaction = 0
        ...
        Person.withTransaction { status1 ->
            ...
            Person.withTransaction { status2 ->
                ...
                results << person2.id
                countDuringNestedTransaction = Person.count()
            }
        }

        then: "both persons should be saved immediately"
        countDuringNestedTransaction == initialCount + 2

        and: "all changes should persist"
    ...
    void "test native session in transaction context"() {
        when: "within native transaction"
        def nativeSession = null
        Person.withTransaction { status ->
            nativeSession = DatastoreUtils.getSession(mongoDatastore)
        }
