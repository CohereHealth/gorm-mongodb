...
class NativeQueryIntegrationSpec extends Specification {
    ...
    void "test aggregation with project via service"() {
        given:
        Procedure.withNativeTransaction { session ->
    ...
    void "test service creates diagnosis with procedures"() {
        ...
        Diagnosis.get(diagnosis.id).procedures.size() == 2
    }

    void "test service finds procedures by diagnosis"() {
        given:
        def diag2Id = Diagnosis.withNativeTransaction { session ->
        ...
        when:
        def diag2 = Diagnosis.get(diag2Id)
    ...
    void "test service finds expensive procedures by diagnosis"() {
        given:
        def diagId = Diagnosis.withNativeTransaction { session ->
    ...
    void "test service removes procedure from diagnosis"() {
        ...
        def initialCount = Procedure.count()

        when:
        def diagnosis = Diagnosis.get(diagId)
        def proc = diagnosis.procedures[0]
        facilityService.removeProcedureFromDiagnosis(diagnosis, proc)

        then:
        Procedure.count() == initialCount - 1
        Diagnosis.get(diagId) != null
    }
